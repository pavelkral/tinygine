...
Texture2D<float4> g_PosMap      : register(t2); // Èteme G-Buffer pozici!
...
cbuffer CloudCB : register(b0)
{
    ...
    float4 camRight; 
    float4 camUp;
    float3 camPosRel;  float  timeSeconds;
    float3 sunDir;     float  planetRadius;
    ...
    float3 cSun; float sunInt;
    float3 cAmbTop; float ambInt;
    float3 cAmbBot; float pad1;
    float4x4 invProj;
    
    float3 camPosAbs; float pad2;
}
...
float ProfileStratus(float h)        { return smoothstep(0.00, 0.08, h) * (1.0 - smoothstep(0.25, 0.55, h)); }
float ProfileStratoCu(float h)       { return smoothstep(0.00, 0.10, h) * (1.0 - smoothstep(0.45, 0.85, h)); }
float ProfileCumulus(float h)        { return smoothstep(0.05, 0.18, h) * (1.0 - smoothstep(0.70, 1.00, h)); }
float ProfileCumulonimbus(float h)   { return smoothstep(0.02, 0.12, h) * (1.0 - smoothstep(0.88, 1.00, h)); }
...
{
    float3 planetCenter = float3(0, -planetRadius - camPosRel.y, 0);
    ...
    if(dist < rMin || dist > rMax) return 0.0;
    ...
    float3 shapePos = float3(localPos.x + shapeOffset.x, localPos.y + camPosRel.y, localPos.z + shapeOffset.y);
...
float4 PSMain(PS_INPUT input) : SV_TARGET
{
    ...
    if (dot(pixelWorldPos, pixelWorldPos) > 0.001) 
    {
        ...
        if (geomDist > 60000.0) geomDist = 1e9;
    }
    ...
    float2 tAtm = RaySphere(-planetCenter, rd, rMax);
    float2 tSrf = RaySphere(-planetCenter, rd, rMin);

    float tStart = max(0.0, tAtm.x);
    float tEnd   = tAtm.y;

    if(tEnd <= 0.0) return float4(0,0,0,0);
    if(tSrf.x > 0.0 && tSrf.x < tEnd) tEnd = tSrf.x;

    if(tStart >= geomDist) return float4(0,0,0,0);
    tEnd = min(tEnd, geomDist);

    float rayLen = tEnd - tStart;
    if(rayLen <= 0.0) return float4(0,0,0,0);
