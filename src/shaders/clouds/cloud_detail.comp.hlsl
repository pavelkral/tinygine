...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    float e0 = TileableWorley(uv, f * 1, seed + 101u);
    float e1 = TileableWorley(uv, f * 2, seed + 202u);
    float wx = TileablePerlin(uv.yzx, f * 1, seed + 303u);
    float wz = TileablePerlin(uv.zxy, f * 1, seed + 707u);

    Result[id] = float4(e0, e1, wx, wz);
}