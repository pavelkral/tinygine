...
// --- UNIVERZÁLNÍ FUNKCE PRO VÝPOÈET JEDNOHO SVÌTLA ---
...
{
    float4 albedoColor = input.baseColor;
    if (input.hasAlbedoTex > 0.5f)
        albedoColor *= texAlbedo.Sample(samp, input.uv);

    clip(albedoColor.a - input.alphaCutoff);

    float3 albedo = pow(abs(albedoColor.rgb), 2.2);
    
    float r = input.roughness;
    if (input.hasRoughTex > 0.5f)
        r *= texRough.Sample(samp, input.uv).r;
        
    float m = input.metalness;
    if (input.hasMetalTex > 0.5f)
        m *= texMetal.Sample(samp, input.uv).r;

    float3 N = normalize(input.normal);
    float3 V = normalize(camPos - input.worldPos);
    
    if (input.hasNormalTex > 0.5f)
