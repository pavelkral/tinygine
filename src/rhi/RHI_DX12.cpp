...
std::shared_ptr<RHIPipeline> RHI_DX12::CreatePipeline(const PipelineConfig &config) {
    ...
    // 1. Kompilace Vertex Shaderu p�es modern� DXC
    auto vsBlob = ShaderCompiler::CompileDX12(config.vsPath, "VSMain", "vs_6_0");
    if (!vsBlob) {
        // Pokud sel�e, vrac�me p. (MessageBox s chybou u� vysko�il uvnit� CompileDX12)
        return p;
    }

    // 2. Kompilace Pixel Shaderu (pokud je zad�n)
...
std::shared_ptr<RHIPipeline> RHI_DX12::CreateComputePipeline(const std::wstring& csPath) {
    ...
    // Vyu�it� na�� nov� t��dy s modern�m DXC kompil�torem
    auto csBlob = ShaderCompiler::CompileDX12(csPath, "CSMain", "cs_6_0");
    if (!csBlob) return nullptr; // Pokud kompilace sel�e, nepokra�ujeme!
...
std::shared_ptr<RHITexture> RHI_DX12::CreateUAVTexture3D(int w_in, int h_in, int depth_in, int format) {
    ...
    D3D12_RESOURCE_DESC d = {};
    d.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE3D; // MUS� B�T 3D
    d.Width = w_in;
    d.Height = h_in;
    ...
    D3D12_UNORDERED_ACCESS_VIEW_DESC ud = {};
    ud.Format = fmt;
    ud.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE3D; // MUS� B�T 3D
    ...
    D3D12_SHADER_RESOURCE_VIEW_DESC sd = {};
    sd.Format = fmt;
    sd.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE3D; // MUS� B�T 3D
    sd.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
