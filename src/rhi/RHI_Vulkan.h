...
private:
    ...
    VkRenderPass mainRenderPass =
        VK_NULL_HANDLE; // UI and final output to screen (No Depth)
    VkRenderPass mrtRenderPass =
        VK_NULL_HANDLE; // 3D Scene G-Buffer (Colors, Normals, Positions + Depth)
    VkRenderPass shadowRenderPass =
        VK_NULL_HANDLE; // Directional Light Shadows (Depth only)
    VkRenderPass singleOffscreenRenderPass =
        VK_NULL_HANDLE; // Post-processing passes (1 Color, No Depth)
    ...
    std::vector<VkSemaphore>
        imageAvailableSemaphores; // Wait for monitor to return a free image
    std::vector<VkFence>
        inFlightFences; // Prevent CPU from overwriting currently rendering frame
    std::vector<VkSemaphore>
        renderFinishedSemaphores; // Allocated to 8: Safely maps to the physical
...
public:
    ...
    std::vector<uint32_t> CompileHLSL(const std::wstring &path, const char *entry,
                                      const char *target);
    ...
    std::shared_ptr<RHIBuffer> CreateBuffer(BufferType t, const void *d, size_t s,
